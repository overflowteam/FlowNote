...
function delegateEvent(element, tag, eventname, listener){
	addEvent(element,eventname,function(event){
		...
		}else if (target.parentNode.tagName.toLowerCase() == tag.toLowerCase()) {
			listener.call(target.parentNode,event);
		};
	})
}

q.on = function(selector,event,listener){
	addEvent(q(selector),event,listener);
}

q.un = function(selector,event,listener){
	removeEvent(q(selector),event,listener);
}

q.click = function(selector,listener){
	addClickEvent(q(selector),listener);
}

q.enter = function(selector,listener){
	addEnterEvent(q(selector),listener);
}

q.delegate = function(selector,tag,event,listener){
	delegateEvent(q(selector),tag,event,listener);
}
...
//将json对象存入localStorage
function LSStorageJson(name,json){
	var jsonStr = JSON.stringify(json);
	localStorage.setItem(name,jsonStr);
}
//从localStorage中读取json对象
function LSReadJson(name){
	var jsonStr = localStorage.getItem(name);
	return JSON.parse(jsonStr);
}
q.storageJson = function(name,json){
	LSStorageJson(name,json);
}
q.readStorageJson = function(name){
	return LSReadJson(name);
}